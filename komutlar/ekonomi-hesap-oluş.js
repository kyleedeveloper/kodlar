...
exports.run = async (client, message, args, perms) => {
  ...
  const hesapismi = await db.fetch(`hesapismiulash-${message.author.id}`);
...
if(!hesapismi) {
  ...
  db.set(`hesapdurumulash-${message.author.id}`, "aktif");
      message.channel.send("Hesabınız aktif edildi!")
      if(client.ekoayarlar.rastgelepara == true) {
        db.set(`hesapismiulash-${message.author.id}`, isim)
        const yıl = new Date().getFullYear();
        const ay = new Date().getDate();
        const gün = new Date().getMonth();
        db.set(`hesaptarihyılulash-${message.author.id}`, yıl)
        db.set(`hesaptarihayulash-${message.author.id}`, ay)
        db.set(`hesaptarihgünulash-${message.author.id}`, gün)
        const randomizer = getRandomInt(client.ekoayarlar.minpara, client.ekoayarlar.maxpara)
        db.add(`bakiyeulash-${message.author.id}`, randomizer)
        ...
        if(client.ekoayarlar.rastgelepara == false) {
          ...
          db.set(`hesaptarihayulash-${message.author.id}`, ay)
          db.set(`hesaptarihgünulash-${message.author.id}`, gün)
          db.add(`bakiyeulash-${message.author.id}`, client.ekoayarlar.başlangıçparası)
